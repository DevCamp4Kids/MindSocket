...
public class OrientationManager {
    ...
    private SensorEventListener mSensorListener = new SensorEventListener() {
        ...
        private float previousHeading = -100f;
        ...
        public void onSensorChanged(SensorEvent event) {
            if (event.sensor.getType() == Sensor.TYPE_ROTATION_VECTOR) {
                ...
                if(previousHeading == -100f) {
                    previousHeading = mHeading;
                }

                //System.out.println("Heading : " + mHeading);
                if(c.connected) {
                    float diff = mHeading - previousHeading;
                    System.out.println("Diff : " + diff);

                    if(diff > 10f || diff < -10f) {
                        c.send(String.valueOf(Math.round(4.67777778 * diff * -1)));

                        System.out.println("Rounds : " + 4.67777778 * diff);
    ...
    public OrientationManager(SensorManager sensorManager, LocationManager locationManager) {
        ...
        try {
            ...
            System.out.println("### Catched URISyntaxException ###");
            e.printStackTrace();
        }
    }
